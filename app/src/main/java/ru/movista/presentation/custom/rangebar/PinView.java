...
class PinView extends View {
    ...
    public void init(Context ctx, float y, float pinRadiusDP, int pinColor, int textColor,
                     float circleRadius, int circleColor, int circleBoundaryColor, float circleBoundarySize, float minFont, float maxFont, boolean pinsAreTemporary, int parentWidth, float margin) {
        ...
        mTextPaint.setTypeface(ResourcesCompat.getFont(getContext(), R.font.roboto_bold)); // movista added
        // Creates the paint and sets the Paint values
    ...
    public void draw(Canvas canvas) {
        ...
        // не даем выйти за переделы родителя
        float xFiltered = Math.min(mX, (mParentWidth - mLeftMargin)); // справа

        if (xFiltered < mLeftMargin) { // слева
            xFiltered = mLeftMargin;
        }

        //Draw the circle boundary only if mCircleBoundaryPaint was initialized
        if (mCircleBoundaryPaint != null)
            canvas.drawCircle(xFiltered, mY, mCircleRadiusPx, mCircleBoundaryPaint);
        canvas.drawCircle(xFiltered, mY, mCircleRadiusPx, mCirclePaint);
        ...
        // не даем длинной надписи выйти за пределы родителя
        float textWidth = mTextPaint.measureText(text);
        float xText = xFiltered;
        if ((textWidth / 2) >= xFiltered / 2) {
            xText = xFiltered + ((textWidth / 2) - xFiltered / 2);
        } else if ((textWidth / 2) >= ((mParentWidth - xFiltered) / 2)) {
            xText = xFiltered - ((textWidth / 2) - ((mParentWidth - xFiltered) / 2));
        }

        canvas.drawText(text,
                xText, mY - mPinRadiusPx - mPinPadding + mTextYPadding,
                mTextPaint);
        super.draw(canvas);
    }
